...
class Module:
    ...
    def send(self, destination_module_key):
        ...
        if destination_module_key in modules.keys():
            ...
            # Last module found, get the module that sends to that one
            for sender, receiver in sender_receiver:
                if receiver == destination_module_key:
                    watch_module = sender
...
class Conjunction(Module):
    ...
    def receive(self, signal, sender):
        global watch_module_counter
        global button_push_counter
        global all_found

        self.input_signals[sender] = signal

        if signal == 1:
            if self.name == watch_module:
                if sender not in watch_module_counter.keys():
                    watch_module_counter[sender] = button_push_counter

                if len(watch_module_counter.keys()) == len(self.input_signals.keys()):
                    all_found = True
...
if __name__ == "__main__":
    ...
    result = prod(watch_module_counter.values())
    print(result)
