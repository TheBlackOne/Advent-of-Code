...
def get_support(brick_key, block_coords, bricks, down=False):
    supportees = set()
...
def get_at_xy(brick_key, x, y):
    all_xyz = []
    for test_brick_key, block_coords in global_bricks.items():
        if test_brick_key == brick_key:
            continue
...
if __name__ == "__main__":
    for i, line in enumerate(input.splitlines()):
        ...
        global_bricks[brick_key] = blocks

    print("Settling bricks...")
    # Fall down all bricks
    while True:
        have_fallen = False
        for brick_key, block_coords in global_bricks.items():
            ...
            if can_fall:
                ...
                global_bricks[brick_key] = new_block_coords
                have_fallen = True
