...
def test_pos_for_path(_path_coords, _pos):
    return _pos in _path_coords

def get_free_directions(_path_coords, _pos):
    result = []

    for direction, direction_vec in directions.items():
        pos_to_test = add_vectors(_pos, direction_vec)
        if test_pos_for_path(_path_coords, pos_to_test):
            result.append(direction)

    return result

def get_opposite_direction(_direction):
...
def get_turn(dir1, dir2):
    if dir1 == 3 and dir2 == 0: dir2 = 4
    elif dir1 == 0 and dir2 == 3: dir1 = 4

    if dir2 > dir1: return 'L'
    elif dir2 < dir1: return 'R'

def reached_end(_direction, _pos, _path_coords):
    free_directions = get_free_directions(_path_coords, _pos)
    return len(free_directions) == 1 and free_directions[0] == get_opposite_direction(_direction)

cpu = IntcodeCPU(input.copy())
...
for path_coord in path_coords:
    if all([test_pos_for_path(path_coords, add_vectors(path_coord, direction)) for direction in directions.values()]):
        intersection_alignments.append(path_coord[0] * path_coord[1])

# part 1
print(sum(intersection_alignments))

test = (32, 16)
test2 = reached_end(0, test, path_coords)

movement = []
pos = start_coords
stride = 0
while not reached_end(direction, pos, path_coords):
    pos_ahead = add_vectors(pos, directions[direction])
    if test_pos_for_path(path_coords, pos_ahead):
        stride += 1
        pos = pos_ahead
        if reached_end(direction, pos, path_coords):
            movement.append(str(stride))
    else:
        free_directions = get_free_directions(path_coords, pos)
        if direction not in free_directions:
            if stride > 0:
                movement.append(str(stride))

            opposite_direction = get_opposite_direction(direction)
            if opposite_direction in free_directions:
                free_directions.remove(get_opposite_direction(direction))
            new_direction = free_directions[0]
            movement.append(get_turn(direction, new_direction))
            direction = new_direction
            stride = 0

path_string = ','.join(movement)
print(path_string)