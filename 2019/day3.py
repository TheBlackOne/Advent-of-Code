...
def get_intersection_steps(intersections, wirecoords):
    result = [wirecoords.index(value) + 1 for value in intersections]
    return result

def get_list_sums(lst1, lst2):
    result = [x + y for x, y in zip(lst1, lst2)]
    return result

def get_distances(lst):
    result = [abs(value[0]) + abs(value[1]) for value in lst]
    return result
...
def main():
    ...
    fill_wirepath(wirepath1, wirecoords1)
    fill_wirepath(wirepath2, wirecoords2)

    intersections = intersection(wirecoords1, wirecoords2)
    distances = get_distances(intersections)

    # part 1
    min_distance = min(distances)
    print("Shortest Manhattan distance: {}".format(min_distance))

    # part 2
    intersection_steps1 = get_intersection_steps(intersections, wirecoords1)
    intersection_steps2 = get_intersection_steps(intersections, wirecoords2)
    combined_steps = get_list_sums(intersection_steps1, intersection_steps2)
    min_steps = min(combined_steps)
    print("Shortest travel distance: {}".format(min_steps))

    end = time.time()
    print('Elapsed time: {} sec'.format(round(end - start, 2)))

    process = psutil.Process(os.getpid())
    print("Memory used: {} MB".format(round(process.memory_info().rss / 1024 / 1024, 2)))
